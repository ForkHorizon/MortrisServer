...
func TestPuzzleWaveFunnel_BuildFilterScopesRuns(t *testing.T) {
	pool := testPool(t)
	ctx := context.Background()
	projectID := seedProject(t, pool, false)
	now := time.Now().UTC().Truncate(time.Second)
	...
	seedRevisionWithWaves(t, pool, projectID, 1)
	seedEvents(t, pool, projectID, []seedEvent{
		{EventID: "e0000000-0000-4000-8000-000000000001", InstallID: install, SessionID: "99999999-9999-4999-8999-999999999999", Sequence: 1, Name: "wave_attempt_started", Kind: "product", EffectiveAt: now, BuildNumber: "build-a",
			Properties: map[string]any{"city_id": 1, "house_id": 1, "wave_index": 0, "house_run_id": "run-a", "attempt_id": "attempt-a"}},
		{EventID: "e0000000-0000-4000-8000-000000000002", InstallID: install, SessionID: "99999999-9999-4999-8999-999999999999", Sequence: 2, Name: "wave_attempt_started", Kind: "product", EffectiveAt: now.Add(time.Second), BuildNumber: "build-b",
			Properties: map[string]any{"city_id": 1, "house_id": 1, "wave_index": 0, "house_run_id": "run-b", "attempt_id": "attempt-b"}},
	})

	build := "build-a"
	funnel, err := GetPuzzleWaveFunnel(ctx, pool, projectID, 1, 1, now.Add(-time.Minute), now.Add(time.Hour), ScopeNaturalOnly, &build)
	if err != nil {
		t.Fatalf("GetPuzzleWaveFunnel: %v", err)
	}
	if funnel.Steps[0].Entered != 1 {
		t.Errorf("build-a entered = %d, want 1", funnel.Steps[0].Entered)
	}
}
