...
func TestLoadGameplaySummary_CapsLongBackgroundGap(t *testing.T) {
	...
	seedEvents(t, pool, projectID, []seedEvent{
		{EventID: "f1111111-1111-4111-8111-111111111111", InstallID: installID, SessionID: "b1111111-1111-4111-8111-111111111111", Sequence: 1, Name: "wave_started", Kind: "product", EffectiveAt: now, Properties: props(0)},
		{EventID: "f2222222-2222-4222-8222-222222222222", InstallID: installID, SessionID: "b1111111-1111-4111-8111-111111111111", Sequence: 2, Name: "app_backgrounded", Kind: "product", EffectiveAt: now.Add(time.Minute), Properties: props(60_000)},
		// Resumed 3 days later and completed — without capping this alone would
		// add ~3 days to wall time. Ends in wave_completed so the attempt is
		// "settled" and its duration counts toward the top-level summary
		// (see TestLoadGameplayOutcomes_ClassifiesAttempts for the bucketing itself).
		{EventID: "f3333333-3333-4333-8333-333333333333", InstallID: installID, SessionID: "b2222222-2222-4222-8222-222222222222", Sequence: 1, Name: "wave_completed", Kind: "product", EffectiveAt: now.Add(time.Minute + 72*time.Hour), Properties: props(120_000)},
	})
...
func TestLoadGameplayOutcomes_ClassifiesAttempts(t *testing.T) {
	...
	props := func(attemptID string, activeMs int64) map[string]any {
		return map[string]any{
	...
	seedEvents(t, pool, projectID, []seedEvent{
		...
		{EventID: "a1111111-1111-4111-8111-111111111111", InstallID: installID, SessionID: "b1111111-1111-4111-8111-111111111111", Sequence: 1, Name: "wave_started", Kind: "product", EffectiveAt: now.Add(-2 * time.Hour), Properties: props("attempt-completed", 0)},
		{EventID: "a2222222-2222-4222-8222-222222222222", InstallID: installID, SessionID: "b1111111-1111-4111-8111-111111111111", Sequence: 2, Name: "wave_completed", Kind: "product", EffectiveAt: now.Add(-2*time.Hour + time.Minute), Properties: props("attempt-completed", 60_000)},
		// gave_up: closed without ever completing the wave.
		{EventID: "a3333333-3333-4333-8333-333333333333", InstallID: installID, SessionID: "b3333333-3333-4333-8333-333333333333", Sequence: 1, Name: "wave_started", Kind: "product", EffectiveAt: now.Add(-3 * time.Hour), Properties: props("attempt-gave-up", 0)},
		{EventID: "a4444444-4444-4444-8444-444444444444", InstallID: installID, SessionID: "b3333333-3333-4333-8333-333333333333", Sequence: 2, Name: "attempt_closed", Kind: "product", EffectiveAt: now.Add(-3*time.Hour + time.Minute), Properties: props("attempt-gave-up", 30_000)},
		// in_progress: no terminal event, last event just a minute ago.
		{EventID: "a5555555-5555-4555-8555-555555555555", InstallID: installID, SessionID: "b4444444-4444-4444-8444-444444444444", Sequence: 1, Name: "wave_started", Kind: "product", EffectiveAt: now.Add(-time.Minute), Properties: props("attempt-in-progress", 0)},
		// lost: no terminal event, last event well past lostThresholdHours.
		{EventID: "a6666666-6666-4666-8666-666666666666", InstallID: installID, SessionID: "b5555555-5555-4555-8555-555555555555", Sequence: 1, Name: "wave_started", Kind: "product", EffectiveAt: now.Add(-48 * time.Hour), Properties: props("attempt-lost", 0)},
	})
