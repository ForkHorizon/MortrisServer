...
func GetPuzzleWaveFunnel(ctx context.Context, pool *pgxpool.Pool, projectID string, cityID, houseID int, from, to time.Time, scope TrafficScope) (*PuzzleWaveFunnel, error) {
	...
	entered, err := queryWaveEntryAndCompletion(ctx, pool, projectID, cityID, houseID, from, to, scope)
	if err != nil {
		return nil, err
	}
	...
	abandoned, err := queryWaveAbandonment(ctx, pool, projectID, cityID, houseID, from, to, scope)
	if err != nil {
		return nil, err
	}
...
func queryWaveEntryAndCompletion(ctx context.Context, pool *pgxpool.Pool, projectID string, cityID, houseID int, from, to time.Time, scope TrafficScope) (map[int][2]int64, error) {
	...
	rows, err := pool.Query(ctx, query, projectID, cityID, houseID, from, to)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func queryWaveAbandonment(ctx context.Context, pool *pgxpool.Pool, projectID string, cityID, houseID int, from, to time.Time, scope TrafficScope) (map[int][2]int64, error) {
	...
	SELECT max_wave,
	       COUNT(*) FILTER (WHERE NOT completed AND now()-last_event_at >= make_interval(hours=>$6::int)),
	       COUNT(*) FILTER (WHERE NOT completed AND now()-last_event_at < make_interval(hours=>$6::int))
	FROM progress GROUP BY 1`
	rows, err := pool.Query(ctx, query, projectID, cityID, houseID, from, to, lostThresholdHours)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
