...
func loadPuzzleHouseInsights(ctx context.Context, pool *pgxpool.Pool, detail *PuzzleHouseDetail, projectID string, from, to time.Time, scope TrafficScope) error {
	events, err := loadPuzzleMetricEvents(ctx, pool, projectID, detail.CityID, detail.HouseID, from, to, scope)
	if err != nil {
		return err
	}
...
func loadPuzzleMetricEvents(ctx context.Context, pool *pgxpool.Pool, projectID string, cityID, houseID int, from, to time.Time, scope TrafficScope) ([]puzzleMetricEvent, error) {
