...
func loadGameplaySummary(ctx context.Context, pool *pgxpool.Pool, result *GameplayDiagnostics, projectID string, from, to time.Time, filter GameplayFilter) error {
	...
	// capped at pauseGapCapMS (see its doc comment).
	return pool.QueryRow(ctx, `WITH e AS (SELECT name, effective_at, properties FROM events WHERE project_id=$1 AND event_kind='product' AND effective_at >= $2 AND effective_at < $3 AND ($4::int IS NULL OR (properties->>'city_id')::int=$4) AND ($5::int IS NULL OR (properties->>'house_id')::int=$5) AND ($6::int IS NULL OR (properties->>'wave_index')::int=$6)), g AS (SELECT properties->>'attempt_id' attempt_id, name, (properties->>'active_elapsed_ms')::bigint active_elapsed_ms, LEAST(GREATEST(EXTRACT(EPOCH FROM effective_at - LAG(effective_at) OVER (PARTITION BY properties->>'attempt_id' ORDER BY effective_at))*1000, 0), $7::bigint) gap_ms FROM e WHERE properties ? 'attempt_id'), a AS (SELECT attempt_id, MAX(active_elapsed_ms) active_ms, COALESCE(SUM(gap_ms),0) wall_ms, COUNT(*) FILTER (WHERE name='app_backgrounded') pauses FROM g GROUP BY 1) SELECT (SELECT COUNT(DISTINCT attempt_id) FROM a), (SELECT COUNT(*) FROM e WHERE name='placement_resolved'), (SELECT COUNT(*) FROM e WHERE name='placement_resolved' AND properties->>'outcome' LIKE 'fell_%'), (SELECT COUNT(*) FROM e WHERE name='hint_used'), (SELECT COUNT(*) FROM e WHERE name='wave_completed'), (SELECT COUNT(*) FROM e WHERE name='house_completed'), COALESCE((SELECT SUM(active_ms) FROM a),0), COALESCE((SELECT SUM(wall_ms) FROM a),0)::bigint, COALESCE((SELECT SUM(pauses) FROM a),0), COALESCE((SELECT SUM(GREATEST(wall_ms-active_ms,0)) FROM a),0)::bigint`, projectID, from, to, filter.CityID, filter.HouseID, filter.WaveIndex, pauseGapCapMS).Scan(&result.Summary.Attempts, &result.Summary.Placements, &result.Summary.Falls, &result.Summary.Hints, &result.Summary.CompletedWaves, &result.Summary.CompletedHouses, &result.Summary.ActiveElapsedMS, &result.Summary.WallElapsedMS, &result.Summary.PauseCount, &result.Summary.PauseElapsedMS)
}
