...
export function HousesPage() {
  ...
  const fetchHouses = useCallback(() => apiGet<PuzzleHouseList>('/api/v1/analytics/gameplay/houses', { project: currentProject, from, to, build: build || undefined, scope }), [currentProject, from, to, build, scope])
  const fetchOverview = useCallback(() => apiGet<PuzzleOverview>('/api/v1/analytics/gameplay/overview', { project: currentProject, from, to, build: build || undefined, scope }), [currentProject, from, to, build, scope])
  ...
  const houses = useApiData(fetchHouses, `puzzle-houses:${currentProject}:${from}:${to}:${build}:${scope}`)
  const overview = useApiData(fetchOverview, `puzzle-overview:${currentProject}:${from}:${to}:${build}:${scope}`)
